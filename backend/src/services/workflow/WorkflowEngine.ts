...
import { ExecutionStrategy, ExecutionStrategyFactory } from './ExecutionStrategy';
import { EventSystem, EventSystemFactory } from './EventSystem';
import { WorkflowNode, WorkflowNodeFactory } from './WorkflowNode';
...
/**
 * Core Workflow Engine with Enhanced Execution Strategies
 * Manages workflow execution with pluggable strategies and advanced node support
 */
export class WorkflowEngine {
  ...
  constructor(
    ...
    this.eventSystem = options.eventSystem || EventSystemFactory.create('standard', 'workflow-engine');
    this.executionStrategy = options.executionStrategy || ExecutionStrategyFactory.create('adaptive');
  ...
  /**
   * Execute a workflow using the configured execution strategy
   */
  async executeWorkflow(
    ...
    try {
      ...
      // Set up nodes for this workflow
      this.initializeWorkflowNodes(workflow, context);
      ...
      context.logger.info('Starting workflow execution with strategy', {
        workflowId: workflow.id,
        executionId: execution.id,
        strategy: this.executionStrategy.name
      });

      // Execute using the configured strategy
      const strategyResult = await this.executionStrategy.execute(
        workflow,
        execution,
        context,
        this.eventSystem
      ...
      execution.status = strategyResult.status;
      execution.output_data = strategyResult.outputData;
      execution.error_message = strategyResult.errorMessage;
      execution.completed_at = new Date();

      // Store execution statistics
      execution.metadata = {
        ...(execution.metadata || {}),
        executionStats: strategyResult.executionStats,
        stateHistory: strategyResult.stateHistory
      };

      context.logger.info('Workflow execution completed', {
        status: execution.status,
        duration: execution.completed_at.getTime() - execution.started_at.getTime(),
        statesExecuted: strategyResult.executionStats.statesExecuted
      });
      ...
      await this.eventSystem.emit('workflow.failed', {
        executionId: execution.id,
        workflowId: workflow.id,
      ...
      this.cleanupWorkflowNodes(workflow.id);
    }
  }
   ...
   */
  setExecutionStrategy(strategy: ExecutionStrategy | string): void {
    if (typeof strategy === 'string') {
      this.executionStrategy = ExecutionStrategyFactory.create(strategy);
    } else {
  ...
  off(eventType: string, handler: (event: any) => void): void {
    // Note: EventSystem tracks subscriptions by ID, so this is a simplified approach
    // In production, you'd want to track subscription IDs
    console.warn('Event unsubscription requires subscription ID - use EventSystem directly for precise control');
  }

  /**
   * Wait for a specific event
   */
  waitForEvent<T = any>(eventType: string, timeout?: number): Promise<T> {
    return this.eventSystem.waitFor(eventType, timeout);
  }
  ...
  /**
   * Get workflow execution estimate
   */
  getExecutionEstimate(workflow: WorkflowDefinition): any {
    return this.executionStrategy.estimate(workflow);
  }

  /**
   * Validate workflow can be executed by current strategy
   */
  canExecuteWorkflow(workflow: WorkflowDefinition): boolean {
    return this.executionStrategy.canExecute(workflow);
  }

  /**
   * Get event system for advanced event handling
   */
  ...
  private setupErrorHandling(): void {
    // Global error handler for uncaught errors in event processing
    this.eventSystem.subscribe('error.occurred', async (event) => {
      console.error('Workflow engine error:', event.data);
      
      // Could implement additional error handling logic here
      // such as alerting, logging to external systems, etc.
    }, {
  ...
  /**
   * Initialize workflow nodes for execution
   */
  private initializeWorkflowNodes(workflow: WorkflowDefinition, context: ExecutionContext): void {
    ...
    workflow.states.forEach(state => {
      try {
        const node = WorkflowNodeFactory.create(
          state.type || 'task',
          state.id,
          state.name,
          {
            actions: state.actions,
            condition: (state as any).condition,
            loopType: (state as any).loopType,
            childNodeIds: (state as any).childNodeIds
          },
          {
        ...
        context.logger.warn(`Failed to create node for state ${state.id}:`, error as Error);
        // Continue with other nodes - the execution strategy will handle missing nodes appropriately
      }
    });
  }
   ...
   */
  private cleanupWorkflowNodes(workflowId: string): void {
    // Clean up nodes for this workflow
    const nodesToRemove: string[] = [];
    
    this.nodes.forEach((node, nodeId) => {
      if (node.getMetadata().workflowId === workflowId) {
  ...
  async cleanup(): Promise<void> {
    ...
    if (this.executionStrategy.cleanup) {
      try {
        await this.executionStrategy.cleanup();
      } catch (error) {
        console.warn(`Execution strategy cleanup failed:`, error);
      }
    }
...
export interface WorkflowEngineOptions {
  ...
  executionStrategy?: ExecutionStrategy | string;
