...
test.describe('Educational Workflow Execution', () => {
  let page: Page;
  ...
  test.describe('Workflow Discovery and Selection', () => {
    test('should display available educational workflows', async () => {
      // Mock workflows API
      ...
      const workflowCards = page.locator(
        '[data-testid="workflow-card"], .workflow-item, .course-card'
      );
      ...
      await expect(firstWorkflow).toContainText(/Jung|Psicologia|Analítica/i);
    ...
    test('should filter workflows by difficulty and type', async () => {
      await helpers.mockApiResponse('workflows', {
        workflows: [
      ...
      const difficultyFilter = page.locator(
        '[data-testid="difficulty-filter"], select[name="difficulty"], .filter-difficulty'
      );
      ...
      if (await difficultyFilter.isVisible()) {
        ...
        if (count > 0) {
          ...
          await expect(firstVisible).toContainText(/beginner|iniciante/i);
        }
      }
    });
  });

  test.describe('Workflow Enrollment and Starting', () => {
    test('should enroll student in selected workflow', async () => {
      ...
      const workflowTitle = page.locator(
        '[data-testid="workflow-title"], h1, .workflow-header'
      );
      await expect(workflowTitle.first()).toBeVisible();

      // Find and click enroll button
      const enrollButton = page.locator(
        '[data-testid="enroll-button"], button:has-text("Matricular"), button:has-text("Inscrever")'
      );
      ...
      if (await enrollButton.isVisible()) {
        ...
        const startButton = page.locator(
          'button:has-text("Iniciar"), button:has-text("Continuar")'
        );
        await expect(startButton.first()).toBeVisible();
      }
    });

    test('should start enrolled workflow', async () => {
      ...
      const startButton = page.locator(
        '[data-testid="start-workflow"], button:has-text("Iniciar")'
      );
      ...
      if (await startButton.isVisible()) {
        ...
        await expect(page).toHaveURL(/\/workflow\/1\/(step|module)\/1/);
        
        // Should show workflow progress
        const progress = page.locator(
          '[data-testid="workflow-progress"], .progress-bar, .step-indicator'
        );
  ...
  test.describe('Workflow Step Execution', () => {
    test('should complete workflow steps sequentially', async () => {
      ...
      const stepContent = page.locator(
        '[data-testid="step-content"], .workflow-content, .lesson-content'
      );
      await expect(stepContent).toBeVisible();

      // Should show step title
      const stepTitle = page.locator('h1, h2, .step-title');
      await expect(stepTitle.first()).toContainText(/Introduction|Introdução/i);
      ...
      const completeButton = page.locator(
        '[data-testid="complete-step"], button:has-text("Concluir"), button:has-text("Próximo")'
      );
      ...
      if (await completeButton.isVisible()) {
        ...
        const url = page.url();
        expect(url).toMatch(/step\/2|completed|next/);
      }
    });

    test('should handle interactive workflow elements', async () => {
      ...
      const interactiveContent = page.locator(
        '[data-testid="interactive-content"], .quiz-container, .exercise'
      );
    ...
    test('should save and resume workflow progress', async () => {
      // Mock progress save
      ...
      const resumeButton = page.locator(
        'button:has-text("Continuar"), button:has-text("Resume"), [data-testid="resume-workflow"]'
      );
  ...
  test.describe('Assessment and Evaluation', () => {
    test('should complete workflow assessments', async () => {
      ...
      await expect(assessmentTitle.first()).toContainText(/Quiz|Avaliação|Test/i);
      ...
      if (await firstOption.isVisible()) {
        ...
        const submitButton = page.locator(
          '[data-testid="submit-assessment"], button:has-text("Enviar")'
        );
        ...
        if (await submitButton.isVisible()) {
          ...
          const results = page.locator(
            '[data-testid="assessment-results"], .quiz-results, .score'
          );
          await expect(results.first()).toBeVisible();
          
          // Should show score
          await expect(results.first()).toContainText(/85|8[0-9]/);
        }
      }
    });

    test('should provide feedback and recommendations', async () => {
      ...
      if (page.url().includes('404')) {
        ...
        const viewResultsButton = page.locator(
          'button:has-text("Ver Resultados"), button:has-text("Results")'
        );
      ...
      const completionSummary = page.locator(
        '[data-testid="completion-summary"], .results-summary, .completion-stats'
      );
      
      if (await completionSummary.first().isVisible()) {
        await expect(completionSummary.first()).toBeVisible();
        
        // Should show recommendations
        const recommendations = page.locator(
          '[data-testid="recommendations"], .next-steps, .suggestions'
        );
  ...
  test.describe('Collaborative Features', () => {
    test('should enable discussion during workflow', async () => {
      await page.goto('/workflow/1/step/2');
      await helpers.waitForPageReady();

      // Look for discussion or comment features
      const discussionButton = page.locator(
        '[data-testid="discussion"], button:has-text("Discussão"), .comments-toggle'
      );
      ...
      if (await discussionButton.isVisible()) {
        ...
        const discussionPanel = page.locator(
          '[data-testid="discussion-panel"], .comments-section, .discussion'
        );
        await expect(discussionPanel).toBeVisible();
        
        // Should allow posting comments
        const commentInput = page.locator(
          '[data-testid="comment-input"], textarea[placeholder*="coment"]'
        );
        if (await commentInput.isVisible()) {
          ...
          if (await postButton.isVisible()) {
            ...
            await expect(newComment).toContainText('fascinating');
          }
        }
      }
    });
  ...
  test.describe('Performance and Analytics', () => {
    test('should track detailed learning analytics', async () => {
      ...
      if (page.url().includes('404')) {
        ...
        const analyticsLink = page.locator(
          'a:has-text("Progresso"), a:has-text("Analytics"), .progress-link'
        );
      ...
      const analyticsCharts = page.locator(
        '[data-testid="analytics-chart"], .chart, .progress-chart'
      );
    ...
    test('should provide performance insights', async () => {
      ...
      if (!page.url().includes('404')) {
        ...
        const insights = page.locator(
          '[data-testid="performance-insights"], .insights, .recommendations'
        );
        await expect(insights.first()).toBeVisible();
        
        // Should show personalized recommendations
        await expect(insights.first()).toContainText(/advanced|continue|recommend/i);
      }
    });
  });
  ...
  test.describe('Mobile Workflow Experience', () => {
    ...
    test('should adapt workflow interface for mobile', async () => {
      await page.goto('/workflow/1/step/1');
      await helpers.waitForPageReady();
