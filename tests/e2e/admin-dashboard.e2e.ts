...
test.describe('Admin Dashboard - System Management and Controls', () => {
  let page: Page;
  ...
  test.describe('Admin Authentication and Access', () => {
    test('should access admin dashboard with proper credentials', async () => {
  ...
  test.describe('User Management', () => {
    test('should display list of all users', async () => {
      await page.goto('/admin/users');
      
      if (!page.url().includes('404')) {
        // Verify users table loads
        const usersTable = page.locator(
          '[data-testid="users-table"], .users-table, table'
        );
    ...
    test('should search and filter users', async () => {
      ...
      if (!page.url().includes('404')) {
        ...
        const searchInput = page.locator(
          '[data-testid="user-search"], [name="search"], input[placeholder*="buscar"]'
        );
        ...
        if (await searchInput.isVisible()) {
          ...
          if (await searchResults.count() > 0) {
            ...
            expect(resultText?.toLowerCase()).toContain('student');
          }
        }

        // Filter by role
        const roleFilter = page.locator(
          '[data-testid="role-filter"], [name="role"], select'
        );
    ...
    test('should create new user account', async () => {
      ...
      if (!page.url().includes('404')) {
        const createUserButton = page.locator(
          '[data-testid="create-user"], button:has-text("Criar"), button:has-text("Novo Usuário")'
        );
        ...
        if (await createUserButton.isVisible()) {
          ...
          const saveButton = page.locator(
            '[data-testid="save-user"], button:has-text("Salvar")'
          );
    ...
    test('should edit existing user', async () => {
      ...
      if (!page.url().includes('404')) {
        const editButton = page.locator(
          '[data-testid="edit-user"], .edit-button, button:has-text("Editar")'
        ).first();
    ...
    test('should deactivate/suspend user account', async () => {
      await page.goto('/admin/users');
      
      if (!page.url().includes('404')) {
        const suspendButton = page.locator(
          '[data-testid="suspend-user"], button:has-text("Suspender"), .suspend-button'
        ).first();
        ...
        if (await suspendButton.isVisible()) {
          ...
          const confirmButton = page.locator(
            '[data-testid="confirm-suspend"], button:has-text("Confirmar")'
          );
  ...
  test.describe('System Settings and Configuration', () => {
    test('should access system settings', async () => {
      ...
      if (!page.url().includes('404')) {
        ...
        const settingsForm = page.locator(
          '[data-testid="settings-form"], .settings-form, form'
        );
    ...
    test('should update system configuration', async () => {
      ...
      if (!page.url().includes('404')) {
        ...
        const siteNameInput = page.locator(
          '[data-testid="site-name"], [name="siteName"], input[placeholder*="nome"]'
        );
        ...
        const contactEmailInput = page.locator(
          '[data-testid="contact-email"], [name="contactEmail"]'
        );
        ...
        const featureToggle = page.locator(
          '[data-testid="feature-toggle"], input[type="checkbox"]'
        ).first();
        ...
        const saveSettingsButton = page.locator(
          '[data-testid="save-settings"], button:has-text("Salvar Configurações")'
        );
    ...
    test('should manage API keys and integrations', async () => {
      ...
      if (page.url().includes('404')) {
        ...
        const integrationsTab = page.locator(
          '[data-testid="integrations-tab"], .tab:has-text("Integração")'
        );
      ...
      const openaiKeyInput = page.locator(
        '[data-testid="openai-key"], [name="openaiApiKey"]'
      );
      ...
      if (await openaiKeyInput.isVisible()) {
        ...
        const testConnectionButton = page.locator(
          '[data-testid="test-openai"], button:has-text("Testar")'
        );
      ...
      const youtubeKeyInput = page.locator(
        '[data-testid="youtube-key"], [name="youtubeApiKey"]'
      );
      ...
      const saveIntegrationsButton = page.locator(
        '[data-testid="save-integrations"], button:has-text("Salvar")'
      );
  ...
  test.describe('Content Management System', () => {
    test('should manage course modules', async () => {
      await page.goto('/admin/modules');
      
      // Verify modules page loads
      const modulesGrid = page.locator(
        '[data-testid="modules-grid"], .modules-grid, .content-grid'
      );
      await expect(modulesGrid).toBeVisible({ timeout: 10000 });
      ...
      if (await selectAllCheckbox.isVisible()) {
        ...
        const bulkActionButton = page.locator(
          '[data-testid="bulk-actions"], button:has-text("Ações")'
        );
    ...
    test('should manage media library', async () => {
      await page.goto('/admin/media');
      
      if (!page.url().includes('404')) {
        // Verify media library loads
        const mediaGrid = page.locator(
          '[data-testid="media-grid"], .media-grid, .files-grid'
        );
        await expect(mediaGrid).toBeVisible();

        // Test file upload
        const uploadButton = page.locator(
          '[data-testid="upload-files"], button:has-text("Upload")'
        );
        ...
        if (await createFolderButton.isVisible()) {
          ...
          if (await folderNameInput.isVisible()) {
            ...
            await expect(folderCreated).toBeVisible({ timeout: 10000 });
          }
        }
      }
    });

    test('should manage site pages and content', async () => {
      await page.goto('/admin/pages');
      
      if (!page.url().includes('404')) {
        // View existing pages
        const pagesList = page.locator(
          '[data-testid="pages-list"], .pages-table, table'
        );
        await expect(pagesList).toBeVisible();

        // Create new page
        const createPageButton = page.locator(
          '[data-testid="create-page"], button:has-text("Nova Página")'
        );
        ...
        if (await createPageButton.isVisible()) {
          ...
          const contentEditor = page.locator(
            '[data-testid="page-content"], .content-editor, textarea'
          );
...
- Processo de individuação
          ...
          // Save page
          const savePageButton = page.locator(
            '[data-testid="save-page"], button:has-text("Salvar")'
          );
  ...
  test.describe('Analytics and Reporting', () => {
    test('should display system overview dashboard', async () => {
      await page.goto('/admin/dashboard');
      
      // Verify dashboard widgets load
      const dashboardWidgets = page.locator(
        '[data-testid="dashboard-widget"], .widget, .stat-card'
      );
    ...
    test('should generate usage reports', async () => {
      ...
      if (!page.url().includes('404')) {
        ...
        const reportTypeSelect = page.locator(
          '[data-testid="report-type"], [name="reportType"], select'
        );
        ...
        const generateButton = page.locator(
          '[data-testid="generate-report"], button:has-text("Gerar Relatório")'
        );
        await generateButton.click();

        // Wait for report generation
        const reportResults = page.locator(
          '[data-testid="report-results"], .report-results'
        );
        await expect(reportResults).toBeVisible({ timeout: 15000 });

        // Test report export
        const exportButton = page.locator(
          '[data-testid="export-report"], button:has-text("Exportar")'
        );
        ...
        if (await exportButton.isVisible()) {
          ...
          expect(download.suggestedFilename()).toMatch(/report.*\.(pdf|xlsx|csv)/);
        }
      }
    });
    ...
    test('should monitor system performance metrics', async () => {
      ...
      if (page.url().includes('404')) {
        ...
        const monitoringTab = page.locator(
          '[data-testid="monitoring-tab"], .tab:has-text("Monitor")'
        );
      ...
      const performanceCharts = page.locator(
        '[data-testid="performance-chart"], .chart, canvas, svg'
      );
      
      if (await performanceCharts.count() > 0) {
        await expect(performanceCharts.first()).toBeVisible();
      }

      // Check system health indicators
      const healthIndicators = page.locator(
        '[data-testid="health-indicator"], .health-status, .status-indicator'
      );
      ...
      if (await healthIndicators.count() > 0) {
        ...
        expect(statusText).toMatch(/healthy|unhealthy|online|offline|good|bad/i);
      }

      // Check for alerts or warnings
      const alertsPanel = page.locator(
        '[data-testid="alerts-panel"], .alerts-panel, .warnings'
      );
  ...
  test.describe('Backup and Maintenance', () => {
    test('should create system backup', async () => {
      ...
      if (!page.url().includes('404')) {
        const createBackupButton = page.locator(
          '[data-testid="create-backup"], button:has-text("Backup")'
        );
        ...
        if (await createBackupButton.isVisible()) {
          ...
          const backupProgress = page.locator(
            '[data-testid="backup-progress"], .backup-progress, .progress'
          );
          
          if (await backupProgress.isVisible()) {
            // Wait for completion
            await expect(backupProgress).toBeHidden({ timeout: 30000 });
          }

          // Verify backup completion
          const backupComplete = page.locator(
            '[data-testid="backup-complete"], .backup-success'
          );
          await expect(backupComplete).toBeVisible({ timeout: 30000 });
        }
      }
    });

    test('should manage system maintenance mode', async () => {
      await page.goto('/admin/maintenance');
      
      if (!page.url().includes('404')) {
        const maintenanceModeToggle = page.locator(
          '[data-testid="maintenance-toggle"], input[type="checkbox"]'
        );
        ...
        if (await maintenanceModeToggle.isVisible()) {
          ...
          await expect(maintenanceStatus.first()).toBeVisible({ timeout: 10000 });
          ...
          await expect(maintenanceDisabled.first()).toBeVisible({ timeout: 10000 });
        }
      }
    });
  });
  ...
  test.describe('Security and Access Control', () => {
    test('should view security audit logs', async () => {
      ...
      if (!page.url().includes('404')) {
        ...
        const logsTable = page.locator(
          '[data-testid="security-logs"], .logs-table, table'
        );
    ...
    test('should manage role-based permissions', async () => {
      ...
      if (!page.url().includes('404')) {
        // View existing roles
        const rolesTable = page.locator(
          '[data-testid="roles-table"], .roles-table, table'
        );
        await expect(rolesTable).toBeVisible();

        // Create new role
        const createRoleButton = page.locator(
          '[data-testid="create-role"], button:has-text("Nova Função")'
        );
