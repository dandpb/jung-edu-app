...
export class TestHelpers {
   ...
   */
  async setupAuth(userType: 'student' | 'instructor' | 'admin'): Promise<void> {
    const userConfigs = {
      student: {
        id: 1,
        name: 'Test Student',
   ...
   */
  async mockMultipleApiResponses(mocks: Array<{endpoint: string, response: any, status?: number}>): Promise<void> {
   ...
   */
  async mockLocalStorage(data: Record<string, any>): Promise<void> {
   ...
   */
  async setupBrowserEnvironment(): Promise<void> {
...
export abstract class BasePage {
  ...
  protected async clickElement(locator: any): Promise<void> {
    await expect(locator).toBeVisible();
    await locator.click();
  }

  protected async fillInput(locator: any, value: string): Promise<void> {
    await expect(locator).toBeVisible();
...
export class CustomAssertions {
  ...
  async toBeResponsive(): Promise<void> {
    const helpers = new TestHelpers(this.page);
    await helpers.testResponsiveDesign();
  }

  async toSupportKeyboardNavigation(): Promise<void> {
    const helpers = new TestHelpers(this.page);
