...
test.describe('Assessment and Quiz Completion', () => {
  let page: Page;
  ...
  test.describe('Quiz Discovery and Access', () => {
    test('should display available quizzes for modules', async () => {
      ...
      const quizCards = page.locator(
        '[data-testid="quiz-card"], .quiz-item, .assessment-card'
      );
      ...
      if (await quizCards.first().isVisible()) {
        ...
        await expect(quizCards.first()).toContainText(/Jung Fundamentals|10 quest/);
        ...
        if (await availableQuiz.isVisible()) {
          await expect(availableQuiz).toBeVisible();
        }
        
        if (await unavailableQuiz.isVisible()) {
          await expect(unavailableQuiz).toBeVisible();
        }
      }
    });

    test('should enforce quiz prerequisites', async () => {
      ...
      const prerequisiteMessage = page.locator(
        '[data-testid="prerequisite-message"], .prerequisite-warning, .access-denied'
      );
      
      if (await prerequisiteMessage.isVisible()) {
        await expect(prerequisiteMessage).toBeVisible();
        await expect(prerequisiteMessage).toContainText(/Complete Module|prerequisit/i);
        
        // Start quiz button should be disabled
        const startButton = page.locator(
          '[data-testid="start-quiz"], button:has-text("Iniciar")'
        );
  ...
  test.describe('Quiz Taking Experience', () => {
    test('should start and complete a multiple choice quiz', async () => {
      // Mock quiz data
      ...
      await expect(quizTitle.first()).toContainText(/Jung Fundamentals/i);

      const instructions = page.locator('[data-testid="instructions"], .quiz-instructions');
      if (await instructions.isVisible()) {
        await expect(instructions).toContainText(/Answer all questions/i);
      }

      // Start the quiz
      const startButton = page.locator(
        '[data-testid="start-quiz"], button:has-text("Iniciar Quiz")'
      );
      ...
      const questionText = page.locator(
        '[data-testid="question-text"], .question, .quiz-question'
      );
      await expect(questionText.first()).toContainText(/collective unconscious/i);
      ...
      const nextButton = page.locator(
        '[data-testid="next-question"], button:has-text("Próxima")'
      );
      ...
      if (await nextButton.isVisible()) {
        ...
        await expect(questionText.first()).toContainText(/NOT a major archetype/i);
      ...
      const submitButton = page.locator(
        '[data-testid="submit-quiz"], button:has-text("Enviar"), button:has-text("Finalizar")'
      );
      ...
      if (await submitButton.isVisible()) {
        ...
        const results = page.locator(
          '[data-testid="quiz-results"], .quiz-score, .results-page'
        );
        await expect(results.first()).toBeVisible();
        
        // Should show score
        await expect(results.first()).toContainText(/100|Excellent/);
      }
    });
    ...
    test('should handle different question types', async () => {
      ...
      // Fill in the blank
      const blankInput = page.locator(
        '[data-testid="fill-blank"], input[type="text"], .fill-blank-input'
      );
      ...
      // Essay question
      const essayInput = page.locator(
        '[data-testid="essay-input"], textarea, .essay-answer'
      );
      if (await essayInput.isVisible()) {
        await essayInput.fill(
          'Individuation is the process by which an individual integrates their conscious and unconscious aspects to achieve psychological wholeness and authentic self-realization.'
        );
    ...
    test('should save progress and allow resume', async () => {
      await page.goto('/quiz/1');
      await helpers.waitForPageReady();
      ...
      const resumeButton = page.locator(
        '[data-testid="resume-quiz"], button:has-text("Continuar")'
      );
  ...
  test.describe('Quiz Results and Feedback', () => {
    test('should display detailed results after completion', async () => {
      ...
      const scoreDisplay = page.locator(
        '[data-testid="quiz-score"], .score-display, .final-score'
      );
      await expect(scoreDisplay.first()).toBeVisible();
      await expect(scoreDisplay.first()).toContainText(/85/);

      // Should show pass/fail status
      const passStatus = page.locator(
        '[data-testid="pass-status"], .passed, .quiz-passed'
      );
      ...
      const breakdown = page.locator(
        '[data-testid="score-breakdown"], .question-breakdown, .results-detail'
      );
      if (await breakdown.isVisible()) {
        await expect(breakdown).toContainText(/17.*20|correct/);
      }

      // Should provide feedback
      const feedback = page.locator(
        '[data-testid="feedback"], .quiz-feedback, .performance-feedback'
      );
      if (await feedback.isVisible()) {
        await expect(feedback).toContainText(/Great job|solid understanding/);
      }
    });

    test('should allow review of incorrect answers', async () => {
      ...
      if (page.url().includes('404')) {
        ...
        const reviewButton = page.locator(
          '[data-testid="review-answers"], button:has-text("Revisar")'
        );
      ...
      if (await reviewSection.first().isVisible()) {
        ...
        if (await explanation.first().isVisible()) {
          await expect(explanation.first()).toContainText(/collective unconscious|universal/);
        }
      }
    });
    ...
    test('should provide learning recommendations', async () => {
      ...
      const recommendations = page.locator(
        '[data-testid="recommendations"], .learning-recommendations, .next-steps'
      );
      ...
      if (await recommendations.first().isVisible()) {
        ...
        if (await studyLink.first().isVisible()) {
          await expect(studyLink.first()).toContainText(/Active Imagination|Psychological Types/);
          ...
          const url = page.url();
          expect(url).toMatch(/module\/|quiz\//);
        }
      }
    });
  });

  test.describe('Adaptive Assessment', () => {
      ...
      if (await correctOption.isVisible()) {
        ...
        if (await nextButton.isVisible()) {
          ...
          if (await questionText.isVisible()) {
            await expect(questionText).toContainText(/Harder question/);
          }
        }
      }
    });
  ...
  test.describe('Assessment Analytics', () => {
    test('should track detailed performance metrics', async () => {
      ...
      if (!page.url().includes('404')) {
        ...
        const analyticsDisplay = page.locator(
          '[data-testid="assessment-analytics"], .quiz-analytics, .performance-metrics'
        );
        
        if (await analyticsDisplay.first().isVisible()) {
          await expect(analyticsDisplay.first()).toBeVisible();
          
          // Should show key metrics
          await expect(analyticsDisplay.first()).toContainText(/82|average|improvement/);
        }
      }
    });

    test('should compare performance across topics', async () => {
      await page.goto('/analytics/topics');
      
      if (!page.url().includes('404')) {
        await helpers.waitForPageReady();

        const topicComparison = page.locator(
          '[data-testid="topic-comparison"], .topic-performance, .subject-analysis'
        );
        
        if (await topicComparison.first().isVisible()) {
          await expect(topicComparison.first()).toBeVisible();
  ...
  test.describe('Mobile Quiz Experience', () => {
    ...
    test('should adapt quiz interface for mobile', async () => {
      await page.goto('/quiz/1');
      await helpers.waitForPageReady();
    ...
    test('should handle mobile quiz navigation', async () => {
      await page.goto('/quiz/1');
      await helpers.waitForPageReady();
