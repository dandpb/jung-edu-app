...
test.describe('Mobile Responsiveness - Cross-Device Compatibility', () => {
  ...
  mobileDevices.forEach(device => {
    test.describe(`${device.name} - ${device.viewport.width}x${device.viewport.height}`, () => {
      test.beforeEach(async ({ browser }) => {
        ...
        test.info().annotations.push({ type: 'device', description: device.name });
      });
      ...
      test('should display responsive navigation menu', async ({ page }) => {
        ...
        if (device.viewport.width <= 768) {
          const mobileMenuButton = page.locator(
            '[data-testid="mobile-menu-toggle"], .mobile-menu-button, .hamburger-menu, button[aria-label*="menu"]'
          );
          ...
          if (await mobileMenuButton.isVisible()) {
            ...
            const mobileNav = page.locator(
              '[data-testid="mobile-navigation"], .mobile-nav, .sidebar-mobile'
            );
          ...
          const desktopNav = page.locator(
            '[data-testid="desktop-navigation"], .desktop-nav, nav:not(.mobile-nav)'
          );
      ...
      test('should adapt content layout for screen size', async ({ page }) => {
        await page.goto('/dashboard');
        ...
        if (paragraphCount > 0) {
          ...
          if (lineHeight !== 'normal') {
            ...
            if (!isNaN(numericLineHeight)) {
              expect(numericLineHeight).toBeGreaterThanOrEqual(1.4);
            }
          }
        }
      });
      ...
      test('should optimize images and media for mobile', async ({ page }) => {
        await page.goto('/dashboard');
        ...
        if (imageCount > 0) {
          ...
          if (await firstImage.isVisible()) {
            ...
            expect(hasAlt).not.toBeNull(); // Should have alt text for accessibility
            ...
            if (hasSrcset || hasSizes) {
              expect(hasSrcset || hasSizes).toBeTruthy();
            }
          }
        }
        ...
        if (videoCount > 0) {
          ...
          if (await firstVideo.isVisible()) {
            ...
            if (videoBox) {
              expect(videoBox.width).toBeLessThanOrEqual(device.viewport.width);
            }
      ...
      test('should handle orientation changes', async ({ page }) => {
        ...
        if (await mainContent.isVisible()) {
          ...
          if (contentBox) {
            expect(contentBox.width).toBeLessThanOrEqual(landscapeViewport.width);
            expect(contentBox.height).toBeLessThanOrEqual(landscapeViewport.height);
          }
        }
        ...
        for (let i = 0; i < samplesToCheck; i++) {
          ...
          if (await element.isVisible()) {
            ...
            if (boundingBox) {
              ...
              expect(boundingBox.x + boundingBox.width).toBeLessThanOrEqual(device.viewport.width + 10);
            }
          }
        }
      });
  ...
  test.describe('Cross-Device Feature Parity', () => {
    test('should maintain functionality across devices', async ({ browser }) => {
      const devices = [
