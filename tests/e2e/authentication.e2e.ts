...
test.describe('Authentication Flow', () => {
  ...
  test.describe('User Registration', () => {
    test('should register new user successfully', async () => {
      ...
      const welcomeMessage = page.locator(
        '[data-testid="welcome-message"], h1:has-text("Bem-vindo"), .welcome-text'
      );
    ...
    test('should show validation errors for invalid data', async () => {
      ...
      const emailError = page.locator(
        '[data-testid="email-error"], .error:has-text("email"), .invalid-feedback'
      );
    ...
    test('should validate password confirmation match', async () => {
      ...
      const passwordError = page.locator(
        '[data-testid="password-error"], .error:has-text("senha"), .error:has-text("password")'
      );
  ...
  test.describe('User Login', () => {
    test('should login with valid credentials', async () => {
      ...
      const dashboardContent = page.locator(
        '[data-testid="dashboard"], .dashboard, h1:has-text("Jung"), h1:has-text("Educação")'
      );
    ...
    test('should show error for invalid credentials', async () => {
      ...
      const errorMessage = page.locator(
        '[data-testid="login-error"], .error, .alert-danger, .notification-error'
      );
      await expect(errorMessage.first()).toBeVisible();
      await expect(errorMessage.first()).toContainText(/invalid|inválid|erro|error/i);
    });
    ...
    test('should handle network errors gracefully', async () => {
      ...
      await expect(networkError.first()).toBeVisible({ timeout: 10000 });
    });
  });
  ...
  test.describe('Session Management', () => {
    test('should persist login session across page reloads', async () => {
      ...
      const userMenu = page.locator(
        '[data-testid="user-menu"], .user-menu, .profile-menu, .user-info'
      );
    ...
    test('should logout user successfully', async () => {
      ...
      const logoutButton = page.locator(
        '[data-testid="logout-button"], button:has-text("Sair"), button:has-text("Logout"), .logout'
      );
    ...
    test('should redirect unauthenticated users to login', async () => {
      ...
      const loginForm = page.locator(
        '[data-testid="login-form"], form:has(input[type="email"]), form:has(input[type="password"])'
      );
  ...
  test.describe('Social Authentication', () => {
    test('should show social login options if available', async () => {
      await page.goto('/login');
      await helpers.waitForPageReady();
      ...
      if (await googleLogin.isVisible() || await facebookLogin.isVisible()) {
        ...
        await expect(socialLogin.first()).toBeVisible();
      }
    });
  });
  ...
  test.describe('Mobile Authentication', () => {
    ...
    test('should handle mobile login layout', async () => {
      await page.goto('/login');
      await helpers.waitForPageReady();
      ...
      if (await emailInput.isVisible() && await passwordInput.isVisible()) {
        ...
        await expect(emailInput).toHaveValue('test@jaquedu.com');
