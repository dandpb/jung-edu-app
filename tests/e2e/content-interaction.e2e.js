...
test_1.test.describe('Educational Content Interaction', () => {
    let page;
    let helpers;
    ...
    test_1.test.describe('Module Content Navigation', () => {
        (0, test_1.test)('should navigate through module sections', async () => {
            // Mock module content
            ...
            await (0, test_1.expect)(moduleTitle.first()).toContainText(/Analytical Psychology|Psicologia Analítica/i);
            const sectionContent = page.locator('[data-testid="section-content"], .section-text, .module-content');
            await (0, test_1.expect)(sectionContent.first()).toBeVisible();
            await (0, test_1.expect)(sectionContent.first()).toContainText(/Carl Jung|psychiatrist/i);
            // Should have section navigation
            const nextButton = page.locator('[data-testid="next-section"], button:has-text("Próximo"), .nav-next');
            if (await nextButton.isVisible()) {
                await nextButton.click();
                // Should advance to next section
                await page.waitForTimeout(1000);
                await (0, test_1.expect)(sectionContent.first()).toContainText(/Key Concepts|Conceitos Chave/i);
        ...
        (0, test_1.test)('should show table of contents and allow section jumping', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
            // Look for table of contents or section menu
            const tocButton = page.locator('[data-testid="toc"], button:has-text("Índice"), .table-of-contents, .sections-menu');
            if (await tocButton.isVisible()) {
                await tocButton.click();
                // Should show section list
                const sectionList = page.locator('[data-testid="section-list"], .toc-section, .section-item');
                await (0, test_1.expect)(sectionList.first()).toBeVisible();
                ...
                if (await section2.isVisible()) {
                    ...
                    if (await currentSection.isVisible()) {
                        await (0, test_1.expect)(currentSection).toBeVisible();
                    }
                }
            }
        });
    });
    test_1.test.describe('Multimedia Content', () => {
        (0, test_1.test)('should play video content with controls', async () => {
            ...
            const videoPlayer = page.locator('[data-testid="video-player"], video, .video-container');
            if (await videoPlayer.isVisible()) {
                await (0, test_1.expect)(videoPlayer).toBeVisible();
                // Should have video controls
                const playButton = page.locator('[data-testid="play-button"], .play-btn, button[aria-label*="play"]');
                if (await playButton.isVisible()) {
                    await playButton.click();
                    // Video should start playing (mock interaction)
                    await page.waitForTimeout(1000);
                    const pauseButton = page.locator('[data-testid="pause-button"], .pause-btn, button[aria-label*="pause"]');
                    if (await pauseButton.isVisible()) {
                        await (0, test_1.expect)(pauseButton).toBeVisible();
                    }
                }
                // Test subtitle toggle
                const subtitleButton = page.locator('[data-testid="subtitles"], .subtitle-btn, button[aria-label*="subtitle"]');
                if (await subtitleButton.isVisible()) {
                    ...
                    if (await subtitleTrack.isVisible()) {
                        await (0, test_1.expect)(subtitleTrack).toBeVisible();
                    }
                }
            }
        });
        (0, test_1.test)('should display interactive visualizations', async () => {
            ...
            const visualization = page.locator('[data-testid="visualization"], .mind-map, .interactive-diagram');
            if (await visualization.isVisible()) {
                await (0, test_1.expect)(visualization).toBeVisible();
                ...
                if (await nodeElement.isVisible()) {
                    ...
                    if (await details.isVisible()) {
                        await (0, test_1.expect)(details).toBeVisible();
                    }
                }
            }
        });
        (0, test_1.test)('should handle audio content with transcript', async () => {
            await page.goto('/module/4/audio');
            if (!page.url().includes('404')) {
                await helpers.waitForPageReady();
                const audioPlayer = page.locator('[data-testid="audio-player"], audio, .audio-container');
                if (await audioPlayer.isVisible()) {
                    await (0, test_1.expect)(audioPlayer).toBeVisible();
                    // Should have transcript toggle
                    const transcriptButton = page.locator('[data-testid="transcript"], button:has-text("Transcrição")');
                    if (await transcriptButton.isVisible()) {
                        await transcriptButton.click();
                        const transcript = page.locator('.transcript, .audio-text');
                        await (0, test_1.expect)(transcript).toBeVisible();
                    }
                }
            }
        });
    });
    test_1.test.describe('Reading and Note-Taking', () => {
        (0, test_1.test)('should allow text highlighting', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
            ...
            if (await textContent.first().isVisible()) {
                ...
                const highlightButton = page.locator('[data-testid="highlight"], button:has-text("Destacar"), .highlight-btn');
                if (await highlightButton.isVisible()) {
                    ...
                    if (await highlightedText.isVisible()) {
                        await (0, test_1.expect)(highlightedText).toBeVisible();
                    }
                }
            }
        });
        (0, test_1.test)('should enable note-taking functionality', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
            // Look for notes panel or button
            const notesButton = page.locator('[data-testid="notes"], button:has-text("Notas"), .notes-toggle');
            if (await notesButton.isVisible()) {
                await notesButton.click();
                // Should show notes panel
                const notesPanel = page.locator('[data-testid="notes-panel"], .notes-sidebar, .note-editor');
                await (0, test_1.expect)(notesPanel).toBeVisible();
                // Should allow adding notes
                const noteInput = page.locator('[data-testid="note-input"], textarea[placeholder*="nota"]');
                if (await noteInput.isVisible()) {
                    await noteInput.fill('This concept about archetypes is fundamental to understanding Jung.');
                    const saveNoteButton = page.locator('button:has-text("Salvar"), button:has-text("Save"), [data-testid="save-note"]');
                    if (await saveNoteButton.isVisible()) {
                        ...
                        await (0, test_1.expect)(savedNote.first()).toContainText('archetypes');
                    }
                }
            }
        });
        (0, test_1.test)('should sync notes across sessions', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
            ...
            if (await notesButton.isVisible()) {
                ...
                if (await existingNote.isVisible()) {
                    await (0, test_1.expect)(existingNote).toContainText('Previously saved note');
                }
            }
        });
    });
    test_1.test.describe('Progress Tracking', () => {
        (0, test_1.test)('should track reading time and engagement', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
            ...
            if (await progressDisplay.first().isVisible()) {
                ...
                (0, test_1.expect)(progressText).toMatch(/\d+:\d+|\d+ min/);
            }
        });
        (0, test_1.test)('should mark content as completed', async () => {
            ...
            const completeButton = page.locator('[data-testid="mark-complete"], button:has-text("Concluir"), .complete-section');
            if (await completeButton.isVisible()) {
                await completeButton.click();
                // Should show completion confirmation
                const completionCheck = page.locator('.completed, .check-mark, [data-testid="section-complete"]');
    ...
    test_1.test.describe('Accessibility Features', () => {
        (0, test_1.test)('should support keyboard navigation', async () => {
            ...
            await (0, test_1.expect)(nextElement).toBeVisible();
        });
        (0, test_1.test)('should provide screen reader support', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
        ...
        (0, test_1.test)('should support text size adjustment', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
            // Look for font size controls
            const fontSizeButton = page.locator('[data-testid="font-size"], button:has-text("A+"), .text-size');
            if (await fontSizeButton.isVisible()) {
                ...
                (0, test_1.expect)(newSize).not.toBe(originalSize);
            }
        });
    });
    test_1.test.describe('Content Search and Discovery', () => {
        (0, test_1.test)('should search within module content', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
            // Look for search functionality
            const searchButton = page.locator('[data-testid="search"], button:has-text("Buscar"), .search-toggle');
            if (await searchButton.isVisible()) {
                await searchButton.click();
                const searchInput = page.locator('[data-testid="search-input"], input[placeholder*="buscar"]');
                ...
                if (await searchResults.first().isVisible()) {
                    await (0, test_1.expect)(searchResults.first()).toBeVisible();
                }
            }
        });
        (0, test_1.test)('should provide content recommendations', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
            ...
            const recommendations = page.locator('[data-testid="recommendations"], .related-content, .suggested-modules');
            if (await recommendations.first().isVisible()) {
                ...
                if (await relatedModule.first().isVisible()) {
                    await (0, test_1.expect)(relatedModule.first()).toContainText(/Advanced|Dream/);
                }
            }
        });
    });
    test_1.test.describe('Mobile Content Experience', () => {
        test_1.test.beforeEach(async ({ browser }) => {
            const context = await browser.newContext({
        ...
        (0, test_1.test)('should adapt content layout for mobile', async () => {
            ...
            (0, test_1.expect)(fontSize).toBeGreaterThanOrEqual(14); // Minimum readable size
        });
        (0, test_1.test)('should handle touch gestures for navigation', async () => {
            await page.goto('/module/1');
            await helpers.waitForPageReady();
            const content = page.locator('.module-content');
            if (await content.isVisible()) {
                ...
                if (contentBox) {
                    ...
                    if (await nextIndicator.isVisible()) {
                        await (0, test_1.expect)(nextIndicator).toBeVisible();
                    }
                }
            }
        });
...
//# sourceMappingURL=content-interaction.e2e.js.map