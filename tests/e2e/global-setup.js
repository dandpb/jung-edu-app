...
async function globalSetup(config) {
    ...
    const authDir = path_1.default.join(__dirname, 'auth');
    const reportsDir = path_1.default.join(__dirname, 'reports');
    const screenshotsDir = path_1.default.join(__dirname, 'screenshots');
    const visualBaselineDir = path_1.default.join(__dirname, 'visual-baselines');
    ...
    try {
        ...
        await adminContext.storageState({ path: path_1.default.join(authDir, 'admin-user.json') });
        ...
        fs_1.default.writeFileSync(path_1.default.join(authDir, 'admin-user.json'), JSON.stringify(minimalAuthState, null, 2));
    }
    ...
    try {
        ...
        await userContext.storageState({ path: path_1.default.join(authDir, 'regular-user.json') });
        ...
        fs_1.default.writeFileSync(path_1.default.join(authDir, 'regular-user.json'), JSON.stringify(minimalAuthState, null, 2));
    }
