...
export class AuthPage extends BasePage {
  ...
  async expectLoginSuccess(): Promise<void> {
    ...
    const welcomeOrDashboard = this.page.locator(
      '[data-testid="dashboard"], .dashboard, h1:has-text("Jung"), h1:has-text("Educação")'
    );
    await expect(welcomeOrDashboard.first()).toBeVisible();
  }

  async expectRegistrationSuccess(): Promise<void> {
    await expect(this.page).toHaveURL(/\/(dashboard|welcome|home)/);
    await expect(this.successMessage.first()).toBeVisible({ timeout: 10000 });
  }

  async expectLoginError(): Promise<void> {
    await expect(this.errorMessage.first()).toBeVisible();
    await expect(this.errorMessage.first()).toContainText(/invalid|inválid|erro|error/i);
  }
  ...
  async expectValidationError(field: 'email' | 'password' | 'name'): Promise<void> {
    const errorSelector = {
