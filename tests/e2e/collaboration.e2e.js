...
test_1.test.describe('Real-time Collaboration - WebSocket Communication Tests', () => {
    let page1;
    let page2;
    ...
    test_1.test.describe('Real-time Discussion Forums', () => {
        (0, test_1.test)('should enable real-time messaging between users', async () => {
            ...
            if (forumExists) {
                ...
                if (await discussionTopic.isVisible()) {
                    ...
                    const messageInput1 = page1.locator('[data-testid="message-input"], textarea[placeholder*="mensagem"], .message-input');
                    if (await messageInput1.isVisible()) {
                        await messageInput1.fill('Esta é uma mensagem de teste em tempo real!');
                        const sendButton1 = page1.locator('[data-testid="send-message"], button:has-text("Enviar")');
                        ...
                        await (0, test_1.expect)(newMessage).toBeVisible({ timeout: 10000 });
                        // User 2 responds
                        const messageInput2 = page2.locator('[data-testid="message-input"], textarea[placeholder*="mensagem"], .message-input');
                        if (await messageInput2.isVisible()) {
                            await messageInput2.fill('Recebido! Comunicação em tempo real funcionando.');
                            const sendButton2 = page2.locator('[data-testid="send-message"], button:has-text("Enviar")');
                            ...
                            await (0, test_1.expect)(responseMessage).toBeVisible({ timeout: 10000 });
                        }
                    }
                }
            }
        ...
        (0, test_1.test)('should show typing indicators', async () => {
            await page1.goto('/forum');
            await page2.goto('/forum');
            ...
            if (forumExists) {
                ...
                if (await discussionTopic.isVisible()) {
                    ...
                    const messageInput1 = page1.locator('[data-testid="message-input"], textarea, .message-input');
                    if (await messageInput1.isVisible()) {
                        await messageInput1.focus();
                        await messageInput1.type('User is typing...', { delay: 100 });
                        // User 2 should see typing indicator
                        const typingIndicator = page2.locator('[data-testid="typing-indicator"], .typing-indicator, .is-typing');
                        if (await typingIndicator.isVisible({ timeout: 5000 })) {
                            await (0, test_1.expect)(typingIndicator).toContainText(/digitando|typing/i);
                        }
                        // Stop typing
                        await messageInput1.clear();
                        // Typing indicator should disappear
                        await (0, test_1.expect)(typingIndicator).toBeHidden({ timeout: 5000 });
                    }
                }
            }
            else {
        ...
        (0, test_1.test)('should handle user presence indicators', async () => {
            await page1.goto('/forum');
            await page2.goto('/forum');
            // Check for online user indicators
            const onlineUsers = page1.locator('[data-testid="online-users"], .online-users, .user-list');
            if (await onlineUsers.isVisible()) {
                ...
                if (await user1Indicator.isVisible()) {
                    await (0, test_1.expect)(user1Indicator).toBeVisible();
                }
                if (await user2Indicator.isVisible()) {
                    await (0, test_1.expect)(user2Indicator).toBeVisible();
                }
            }
            ...
            if (await offlineUser.isVisible()) {
                ...
                (0, test_1.expect)(isOfflineMarked || !(await offlineUser.isVisible())).toBeTruthy();
            }
        });
    });
    test_1.test.describe('Collaborative Learning Features', () => {
        (0, test_1.test)('should enable shared study sessions', async () => {
            ...
            if (moduleExists) {
                ...
                const collaborativeSection = page1.locator('[data-testid="collaborative-section"], .collaboration-panel, .study-group');
                if (await collaborativeSection.isVisible()) {
                    // Start a study session
                    const startSessionButton = page1.locator('[data-testid="start-session"], button:has-text("Iniciar Sessão")');
                    if (await startSessionButton.isVisible()) {
                        await startSessionButton.click();
                        // User 2 should see the session invitation
                        const sessionInvite = page2.locator('[data-testid="session-invite"], .session-notification');
                        if (await sessionInvite.isVisible({ timeout: 10000 })) {
                            ...
                            const sessionActive = page1.locator('[data-testid="active-session"], .session-active');
                            await (0, test_1.expect)(sessionActive).toBeVisible({ timeout: 10000 });
                            const sessionActive2 = page2.locator('[data-testid="active-session"], .session-active');
                            await (0, test_1.expect)(sessionActive2).toBeVisible({ timeout: 10000 });
                        }
                    }
                }
            }
        ...
        (0, test_1.test)('should synchronize progress across users', async () => {
            await page1.goto('/module/1');
            await page2.goto('/module/1');
            const moduleExists = !page1.url().includes('404');
            if (moduleExists) {
                ...
                if (await progressElement.isVisible()) {
                    ...
                    const nextButton = page1.locator('[data-testid="next-step"], button:has-text("Próximo")');
                    if (await nextButton.isVisible()) {
                        ...
                        if (await sharedProgress.isVisible()) {
                            ...
                            if (progress1 && progress2) {
                                (0, test_1.expect)(progress1).toBe(progress2);
                            }
                        }
                    }
                }
        ...
        (0, test_1.test)('should enable collaborative note-taking', async () => {
            await page1.goto('/notes');
            await page2.goto('/notes');
            const notesExists = !page1.url().includes('404');
            if (notesExists) {
                // Create a shared note
                const createNoteButton = page1.locator('[data-testid="create-note"], button:has-text("Nova Nota")');
                if (await createNoteButton.isVisible()) {
                    ...
                    const noteContent = page1.locator('[data-testid="note-content"], textarea, .note-editor');
                    if (await noteContent.isVisible()) {
                        await noteContent.fill('Conceitos importantes sobre arquétipos...');
                        // Enable sharing
                        const shareButton = page1.locator('[data-testid="share-note"], button:has-text("Compartilhar")');
                ...
                if (await sharedNote.isVisible()) {
                    ...
                    if (await editableContent.isVisible()) {
                        await editableContent.fill(await editableContent.inputValue() + '\n\nAdicionado por outro usuário: Processo de individuação...');
                        ...
                        if (content) {
                            (0, test_1.expect)(content).toContain('Adicionado por outro usuário');
                        }
                    }
                }
            }
    ...
    test_1.test.describe('WebSocket Connection Management', () => {
        (0, test_1.test)('should handle connection drops and reconnection', async () => {
            await page1.goto('/forum');
            // Check for WebSocket connection status indicator
            const connectionStatus = page1.locator('[data-testid="connection-status"], .connection-indicator, .websocket-status');
            if (await connectionStatus.isVisible()) {
                // Should show connected status
                await (0, test_1.expect)(connectionStatus).toHaveClass(/connected|online/);
            }
            ...
            if (await connectionStatus.isVisible()) {
                ...
                await (0, test_1.expect)(connectionStatus).toHaveClass(/disconnected|offline/);
            }
        ...
        (0, test_1.test)('should queue messages during disconnection', async () => {
            await page1.goto('/forum');
            await page2.goto('/forum');
            const forumExists = !page1.url().includes('404');
            if (forumExists) {
                ...
                if (await discussionTopic.isVisible()) {
                    ...
                    const messageInput = page1.locator('[data-testid="message-input"], textarea, .message-input');
                    if (await messageInput.isVisible()) {
                        ...
                        if (await sendButton.isVisible()) {
                            ...
                            if (await queuedMessage.isVisible()) {
                                await (0, test_1.expect)(queuedMessage).toBeVisible();
                            }
                            // Restore connection
                            await page1.unroute('**/ws');
                            await page1.unroute('**/websocket');
                            ...
                            await (0, test_1.expect)(sentMessage).toBeVisible({ timeout: 15000 });
                        }
                    }
                }
            }
        ...
        (0, test_1.test)('should handle multiple concurrent connections', async () => {
                ...
                if (forumExists) {
                    ...
                    if (await discussionTopic.isVisible()) {
                        ...
                        if (await onlineUsers.isVisible()) {
                            ...
                            (0, test_1.expect)(userCount).toBeGreaterThanOrEqual(3);
                        }
                        ...
                        if (await input1.isVisible() && await input2.isVisible() && await input3.isVisible()) {
                            ...
                            await Promise.all([
                                (0, test_1.expect)(page1.locator(`.message:has-text("${message2}")`)).toBeVisible({ timeout: 10000 }),
                                (0, test_1.expect)(page1.locator(`.message:has-text("${message3}")`)).toBeVisible({ timeout: 10000 }),
                                (0, test_1.expect)(page2.locator(`.message:has-text("${message1}")`)).toBeVisible({ timeout: 10000 }),
                                (0, test_1.expect)(page2.locator(`.message:has-text("${message3}")`)).toBeVisible({ timeout: 10000 }),
                                (0, test_1.expect)(page3.locator(`.message:has-text("${message1}")`)).toBeVisible({ timeout: 10000 }),
                                (0, test_1.expect)(page3.locator(`.message:has-text("${message2}")`)).toBeVisible({ timeout: 10000 })
                            ]);
                        }
                    }
                }
    ...
    test_1.test.describe('Notification System', () => {
        (0, test_1.test)('should send real-time notifications', async () => {
            await page1.goto('/dashboard');
            await page2.goto('/admin/dashboard');
            // Admin (page2) creates an announcement
            const announcementButton = page2.locator('[data-testid="create-announcement"], button:has-text("Anúncio")');
            if (await announcementButton.isVisible()) {
                ...
                if (await titleInput.isVisible()) {
                    ...
                    if (await contentInput.isVisible()) {
                        ...
                        const notification = page1.locator('[data-testid="notification"], .notification, .alert');
                        if (await notification.isVisible({ timeout: 15000 })) {
                            await (0, test_1.expect)(notification).toContainText('Novo Módulo');
                        }
                    }
                }
            }
        });
        (0, test_1.test)('should handle notification preferences', async () => {
            await page1.goto('/settings/notifications');
            if (!page1.url().includes('404')) {
                // Toggle notification settings
                const emailNotifications = page1.locator('[data-testid="email-notifications"], input[type="checkbox"][name*="email"]');
                if (await emailNotifications.isVisible()) {
                    ...
                    (0, test_1.expect)(newState).toBe(!wasChecked);
                }
                const pushNotifications = page1.locator('[data-testid="push-notifications"], input[type="checkbox"][name*="push"]');
...
//# sourceMappingURL=collaboration.e2e.js.map