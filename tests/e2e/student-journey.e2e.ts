...
test.describe('Student Journey - Complete Learning Experience', () => {
  let page: Page;
  ...
  test.describe('User Registration and Onboarding', () => {
    test('should complete user registration flow', async () => {
      ...
      const welcomeElement = page.locator(
        '[data-testid="welcome-message"], h1:has-text("Bem-vindo"), .welcome'
      );
    ...
    test('should handle registration validation errors', async () => {
      ...
      const errorMessage = page.locator(
        '[data-testid="email-error"], .error, .invalid-feedback'
      );
  ...
  test.describe('Login and Authentication', () => {
    test('should login with valid credentials', async () => {
      ...
      const dashboardContent = page.locator(
        '[data-testid="dashboard"], h1:has-text("Jung"), .dashboard-content'
      );
  ...
  test.describe('Course Navigation and Progress', () => {
    ...
    test('should navigate through course modules', async () => {
      await page.goto('/dashboard');

      // Find and click on first module
      const firstModule = page.locator(
        '[data-testid="module-card"], .module-card, .card:has(.module)'
      ).first();
      ...
      const moduleContent = page.locator(
        '[data-testid="module-content"], .module-content, .content'
      );
      await expect(moduleContent).toBeVisible();

      // Check for navigation elements
      const nextButton = page.locator(
        '[data-testid="next-button"], button:has-text("Próximo")'
      );
    ...
    test('should track quiz completion and scores', async () => {
      await page.goto('/dashboard');
      ...
      const quizSection = page.locator(
        '[data-testid="quiz"], .quiz-section, .quiz'
      );
      
      if (await quizSection.isVisible()) {
        // Start quiz
        const startQuizButton = page.locator(
        ...
        if (questionCount > 0) {
          ...
          const submitButton = page.locator(
            '[data-testid="submit-quiz"], button:has-text("Enviar")'
          );
          ...
          const quizResults = page.locator(
            '[data-testid="quiz-results"], .quiz-results, .results'
          );
    ...
    test('should save and resume progress', async () => {
      ...
      const progressIndicator = page.locator(
        '.progress-indicator, .completion-status, [data-testid="progress"]'
      );
      
      // Progress should be visible on the dashboard
      if (await progressIndicator.first().isVisible()) {
        await expect(progressIndicator.first()).toContainText(/\d+%|\d+\/\d+/);
      }
    });
  });
    ...
    test('should update user profile information', async () => {
      ...
      if (await nameInput.isVisible()) {
        ...
        const saveButton = page.locator(
          '[data-testid="save-profile"], button:has-text("Salvar")'
        );
  ...
  test.describe('Social and Collaborative Features', () => {
    test('should access discussion forums', async () => {
      await page.goto('/forum');
      
      if (!page.url().includes('404')) {
        // Verify forum content loads
        const forumContent = page.locator(
          '[data-testid="forum"], .forum, .discussions'
        );
        await expect(forumContent).toBeVisible();
    ...
    test('should create and post in discussions', async () => {
      await page.goto('/forum');
      
      if (!page.url().includes('404')) {
        // Look for "new topic" or "new discussion" button
        const newTopicButton = page.locator(
          '[data-testid="new-topic"], button:has-text("Nova"), button:has-text("Criar")'
        );
  ...
  test.describe('Mobile Responsiveness', () => {
    ...
    test('should display mobile navigation menu', async () => {
      ...
      const menuButton = page.locator(
        '[data-testid="mobile-menu"], .mobile-menu-button, .hamburger, button[aria-label*="menu"]'
      );
  ...
  test.describe('Performance and Load Times', () => {
    ...
    test('should handle slow network conditions', async ({ context }) => {
