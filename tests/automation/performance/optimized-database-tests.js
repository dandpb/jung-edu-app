...
exports.MockConnection = exports.OptimizedConnectionPool = exports.OptimizedDatabaseTests = void 0;
exports.runOptimizedDatabaseTests = runOptimizedDatabaseTests;
exports.createOptimizedDatabaseTestSuite = createOptimizedDatabaseTestSuite;
...
exports.OptimizedConnectionPool = OptimizedConnectionPool;
class MockConnection {
    ...
    getSimulatedExecutionTime(complexity, paramCount) {
        let baseTime = 5;
        switch (complexity) {
            case 'simple':
                baseTime = 5;
...
class OptimizedDatabaseTests {
     ...
     */
    async executeOptimizedQuery(queryTest) {
        console.log(`  🔍 Testing: ${queryTest.name}`);
        const { duration, memoryGrowth, result } = await (0, memory_efficient_test_utils_1.measureExecution)(`query_${queryTest.name}`, async () => {
     ...
     */
    generateRecommendations(queryResults, memoryUsage) {
        const recommendations = [];
        // Query performance analysis
        const slowQueries = queryResults.filter(r => !r.withinThreshold);
        if (slowQueries.length > 0) {
            recommendations.push(`${slowQueries.length} queries exceeded expected execution time. Consider query optimization.`);
        }
        // Memory usage analysis
        if (memoryUsage.growth > 50) {
            recommendations.push(`Memory growth of ${memoryUsage.growth.toFixed(2)}MB detected. Review query result handling.`);
        }
        if (memoryUsage.peak > this.config.memoryThreshold * 0.8) {
            recommendations.push('Peak memory usage approaching threshold. Consider reducing batch sizes.');
        }
        // Connection pool analysis
        const poolStats = this.connectionPool.getStats();
        if (poolStats.queued > 0) {
            recommendations.push('Connection pool queue detected. Consider increasing pool size or optimizing query duration.');
        }
        // Complex query analysis
        const complexQueries = queryResults.filter(r => r.complexity === 'complex');
        if (complexQueries.some(q => q.executionTime > 100)) {
            recommendations.push('Complex queries taking over 100ms. Review indexes and query plans.');
        }
...
exports.OptimizedDatabaseTests = OptimizedDatabaseTests;
// ============================================================================
// Jest Integration
// ============================================================================
/**
 ...
 */
async function runOptimizedDatabaseTests() {
 ...
 */
function createOptimizedDatabaseTestSuite() {
...
//# sourceMappingURL=optimized-database-tests.js.map