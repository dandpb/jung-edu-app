...
class GlobalCleanupRegistry {
  ...
  private resourceReferences: Map<string, any[]> = new Map();
  private activeHandles: Set<NodeJS.Timeout | NodeJS.Immediate | NodeJS.Timeout> = new Set();
...
export class TestCleanupManager extends EventEmitter {
  private config: CleanupConfig;
  private registry: GlobalCleanupRegistry;
   ...
   */
  async performCleanup(trigger: 'manual' | 'automatic' | 'emergency' | 'interval' = 'manual'): Promise<CleanupReport> {
   ...
   */
  async performEmergencyCleanup(): Promise<CleanupReport> {
   ...
   */
  async cleanupTestResources(testName: string): Promise<void> {
  ...
  private async executeCleanupStrategy(strategy: CleanupStrategy): Promise<CleanupResult> {
    const startTime = performance.now();
    const startMemory = this.getCurrentMemoryUsage();
  ...
  private async performAggressiveCleanup(): Promise<void> {
    ...
    if (typeof global !== 'undefined') {
      ...
      delete (global as any).performanceCache;
      delete (global as any).testDataCache;
      delete (global as any).moduleCache;
    }
  ...
  private addToHistory(report: CleanupReport): void {
 ...
 */
export function getJestCleanupManager(): TestCleanupManager {
 ...
 */
export function setupJestCleanupHooks(): void {
