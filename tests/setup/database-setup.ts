...
import { randomHelpers } from '../utils/test-helpers';
...
export const createTestTables = async (): Promise<void> => {
  const client = await testPool!.connect();
...
export const dropTestTables = async (): Promise<void> => {
  const client = await testPool!.connect();
...
export const seedInitialTestData = async (): Promise<void> => {
  const client = await testPool!.connect();
...
export const seedTestCourse = async (instructorId?: string): Promise<string> => {
  const client = await testPool!.connect();
...
export const createTestUser = async (userData: Partial<any> = {}): Promise<string> => {
  const client = await testPool!.connect();
...
export const getTestUserByRole = async (role: 'student' | 'teacher' | 'admin'): Promise<any> => {
  const users = await executeQuery('SELECT * FROM users WHERE role = $1 LIMIT 1', [role]);
  return users[0] || null;
};
...
export const cleanupTestCourse = async (courseId: string): Promise<void> => {
