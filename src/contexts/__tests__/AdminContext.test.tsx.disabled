...
describe('AdminContext', () => {
  ...
  describe('AdminProvider Initialization', () => {
    ...
    it('should handle corrupted localStorage data gracefully', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Failed to access or parse modules from localStorage:',
  ...
  describe('Authentication', () => {
    ...
    it('should reject incorrect password', async () => {
      ...
      // Check that session token was not set (modules may still be persisted)
      const sessionTokenCalls = localStorageMock.setItem.mock.calls.filter(
        call => call[0] === 'admin_session_token'
      );
      expect(sessionTokenCalls).toHaveLength(0);
    });
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle localStorage setItem errors gracefully', async () => {
      // Mock setItem to throw error only after component initialization
      let throwError = false;
      localStorageMock.setItem.mockImplementation((key: string, value: string) => {
        if (throwError) {
          throw new Error('localStorage quota exceeded');
        }
        // Allow initial module persistence
      });
