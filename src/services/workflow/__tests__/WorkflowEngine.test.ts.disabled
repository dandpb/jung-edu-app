...
describe('WorkflowEngine', () => {
  ...
  describe('Initialization', () => {
    test('should initialize with required dependencies', () => {
      ...
      expect(engine).toHaveProperty('executeWorkflow');
    ...
    test('should throw error if initialized without required dependencies', () => {
      expect(() => new WorkflowEngine(null as any, mockPluginSystem as any, mockServices as any))
  ...
  describe('Workflow Execution', () => {
    test('should execute simple workflow successfully', async () => {
      ...
      const result = await engine.executeWorkflow(sampleWorkflow, {
        userId: 'user-1',
        moduleId: 'module-1'
      });
    ...
    test('should handle workflow execution with input validation', async () => {
      const invalidInput = { moduleId: 'module-1' }; // missing required userId

      await expect(engine.executeWorkflow(sampleWorkflow, invalidInput))
    ...
    test('should track execution events', async () => {
      ...
      await engine.executeWorkflow(sampleWorkflow, {
        userId: 'user-1',
        moduleId: 'module-1'
      });
    ...
    test('should handle plugin execution failures gracefully', async () => {
      mockStateManager.createExecution.mockResolvedValue(sampleExecution);
      mockStateManager.getExecution.mockResolvedValue(sampleExecution);
      mockPluginSystem.execute.mockResolvedValue({ 
        success: false, 
      ...
      expect(mockStateManager.updateExecution).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          status: 'failed',
          error_message: expect.stringContaining('Plugin execution failed')
        })
      );
    });
  });
  ...
  describe('State Machine Logic', () => {
    test('should transition between states correctly', async () => {
      ...
      // Should update state from 'start' to 'process-progress'
      expect(mockStateManager.updateExecution).toHaveBeenCalledWith(
        'execution-1',
        expect.objectContaining({
          current_state: 'process-progress'
        })
      );
    });
    ...
    test('should handle conditional transitions', async () => {
      ...
      await engine.executeWorkflow(workflowWithConditions, {
        userId: 'user-1',
        moduleId: 'module-1'
      });
    ...
    test('should handle parallel state execution', async () => {
      ...
      mockStateManager.createExecution.mockResolvedValue({
        ...sampleExecution,
        current_state: 'parallel-state'
      });
      ...
      const result = await engine.executeWorkflow(parallelWorkflow, {
        userId: 'user-1',
        moduleId: 'module-1'
      });

      expect(mockPluginSystem.execute).toHaveBeenCalledTimes(2);
    });
    ...
    test('should reach final state correctly', async () => {
      ...
      // Should eventually reach the 'end' state
      expect(mockStateManager.updateExecution).toHaveBeenCalledWith(
        'execution-1',
        expect.objectContaining({
          status: 'completed',
  ...
  describe('Plugin System Integration', () => {
    ...
    test('should handle plugin timeout', async () => {
      ...
      const result = await engine.executeWorkflow(workflowWithTimeout, {
        userId: 'user-1',
        moduleId: 'module-1'
      });
    ...
    test('should apply plugin retry policy', async () => {
      ...
      await engine.executeWorkflow(workflowWithRetry, {
        userId: 'user-1',
        moduleId: 'module-1'
      });
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    test('should handle invalid workflow definition', async () => {
      ...
      await expect(engine.executeWorkflow(invalidWorkflow, {
        userId: 'user-1',
        moduleId: 'module-1'
      })).rejects.toThrow(WorkflowError);
    });
    ...
    test('should handle circular transitions', async () => {
      ...
      await expect(engine.executeWorkflow(workflowWithCycle, {
        userId: 'user-1',
        moduleId: 'module-1'
    ...
    test('should handle concurrent execution limits', async () => {
      ...
      await expect(engine.executeWorkflow(sampleWorkflow, {
        userId: 'user-1',
        moduleId: 'module-1'
