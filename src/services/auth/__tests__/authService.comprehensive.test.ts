...
// Mock navigator.userAgent for browser environment
...
describe('AuthService', () => {
  ...
  beforeEach(() => {
    ...
    // Setup JWT mocks
    mockedJWT.createAccessToken.mockResolvedValue('mock-access-token');
    mockedJWT.createRefreshToken.mockResolvedValue('mock-refresh-token');
    mockedJWT.getStoredTokens.mockReturnValue({ accessToken: null, refreshToken: null });
    mockedJWT.storeTokens.mockImplementation(() => {});
    mockedJWT.clearTokens.mockImplementation(() => {});
    mockedJWT.validateToken.mockResolvedValue({ valid: false, payload: null });
  });
  ...
  describe('getCurrentUser', () => {
    ...
    it('should return user when authenticated', async () => {
      ...
      // Mock valid token and stored tokens
      mockedJWT.getStoredTokens.mockReturnValue({
        accessToken: 'valid-token',
        refreshToken: 'valid-refresh'
      });
      mockedJWT.validateToken.mockResolvedValue({
        valid: true,
        payload: { sub: registeredUser.id }
      });
  ...
  describe('refreshToken', () => {
    ...
    it('should refresh access token successfully', async () => {
      ...
      // Mock rotateTokens to return new tokens
      mockedJWT.rotateTokens.mockResolvedValue({
        accessToken: 'new-access-token',
        refreshToken: 'new-refresh-token'
      });

      // Mock validateToken to validate the new access token
      mockedJWT.validateToken.mockResolvedValue({
        valid: true,
        payload: { sub: user.id }
      });
