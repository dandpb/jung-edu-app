...
describe('useI18n Hook', () => {
  beforeEach(() => {
    ...
    mockT.mockImplementation((key: string, options?: any) => {
      ...
      if (safeKey.includes('missing')) return safeKey;
      if (safeKey === 'hello.name' && options?.name) return `Hello ${options.name}`;
      if (safeKey === 'count.items' && options?.count !== undefined) {
        return options.count === 1 ? '1 item' : `${options.count} items`;
      }
      // Don't throw errors in tests - return a safe fallback instead
  ...
  describe('Translation Functionality', () => {
    ...
    it('should handle translation errors gracefully', () => {
      ...
      expect(errorTranslation).toBe('error_fallback_error.key');
    ...
    it('should handle empty and null translation keys', () => {
      ...
      expect(() => {
        const empty = result.current.t('');
        const nullKey = result.current.t(null as any);
        const undefinedKey = result.current.t(undefined as any);

        expect(empty).toBe('translated_');
        expect(nullKey).toBeNull();
        expect(undefinedKey).toBeUndefined();
      }).not.toThrow();
    });
  });
  ...
  describe('Edge Cases and Error Scenarios', () => {
    it('should handle malformed translation keys gracefully', () => {
      ...
      // Handle null and undefined separately since they return their original value
      expect(() => {
        const nullTranslation = result.current.t(null as any);
        expect(nullTranslation).toBeNull();
      }).not.toThrow();

      expect(() => {
        const undefinedTranslation = result.current.t(undefined as any);
