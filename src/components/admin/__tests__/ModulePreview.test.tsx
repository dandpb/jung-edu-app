...
jest.mock('lucide-react', () => {
  ...
  return {
    RefreshCw: ({ className }: any) => React.createElement('svg', {
      'data-testid': 'refresh-icon',
      className: `lucide-refresh-cw ${className || ''}`
    }, 'RefreshCw'),
    Edit3: ({ className }: any) => React.createElement('svg', {
      'data-testid': 'edit-icon',
      className: `lucide-edit3 ${className || ''}`
    }, 'Edit3'),
    ...
    X: ({ className }: any) => React.createElement('svg', {
      'data-testid': 'x-icon',
      className: `lucide-x w-6 h-6 ${className || ''}`
    }, 'X'),
    Plus: ({ className }: any) => React.createElement('svg', {
      'data-testid': 'plus-icon',
      className: `lucide-plus ${className || ''}`
    }, 'Plus'),
    Lightbulb: ({ className }: any) => React.createElement('svg', {
      'data-testid': 'lightbulb-icon',
      className: `lucide-lightbulb ${className || ''}`
    }, 'Lightbulb'),
  };
});
...
describe('ModulePreview', () => {
  ...
  describe('AI Suggestions Panel', () => {
    ...
    it('should close suggestions panel when X clicked', () => {
      ...
      // Find the X button in suggestions panel by test ID
      const xIcons = screen.getAllByTestId('x-icon');
      const suggestionsCloseButton = xIcons[xIcons.length - 1].closest('button');

      if (suggestionsCloseButton) {
