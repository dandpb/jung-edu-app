          ...
          <div className="basic-info" data-testid="basic-content">
            ...
            <div className="form-group">
              ...
              <input
                id="template-tags"
                type="text"
                value={template.metadata?._inputValue || template.metadata?.tags?.join(', ') || ''}
                onChange={(e) => {
                  // Keep the input value as-is for display
                  const inputValue = e.target.value;
                  handleInputChange('metadata', {
                    ...template.metadata,
                    tags: inputValue.split(',').map(tag => tag.trim()).filter(tag => tag),
                    _inputValue: inputValue // Store raw input for display
                  });
                }}
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        setTemplate(prev => ({
                          ...prev,
                          states: prev.states?.map(s => 
                            s.id === state.id ? { ...s, type: e.target.value } : s
                          ) || []
                        }));
                      }}
...
describe('WorkflowTemplateBuilder Component', () => {
  ...
  describe('Basic Rendering', () => {
    test('should render template builder with all tabs', () => {
      render(
        <WorkflowTemplateBuilder onSave={mockOnSave} onCancel={mockOnCancel} />
      );
    ...
    test('should show edit mode when isEditing is true', () => {
      render(
        <WorkflowTemplateBuilder onSave={mockOnSave} onCancel={mockOnCancel} isEditing />
      );
